...
| 720-CAD staging order | At 1,200 rpm, close the valve-enabled mass/species/enthalpy/temperature state first; only then enable friction, crank dynamics/motor control or widen the RPM campaign | The signed bidirectional diagnostic closes mass/species transfer accounting (about 2.1e-14 relative mass and 2.3e-21 kg maximum species residual at 2 CAD), but direct cycling remains a non-monotonic transient; later stages cannot repair a non-periodic state |
...
| Valve model scope | State reverse flow, clipping, timing, area, and manifold assumptions explicitly | The current diagnostic is signed/bidirectional and lumped, with adaptive explicit substeps and a positivity safeguard; it is not a calibrated valve train |
