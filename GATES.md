...
| 720-CAD staging order | At 1,200 rpm, close the valve-enabled mass/species/enthalpy/temperature state first; only then enable friction, crank dynamics/motor control or widen the RPM campaign | The four-cycle preflight's 3.19% homologous mass and 1.14% enthalpy drift prompted accounting. The corrected 12-cycle diagnostic shows a transient state and step-dependent mass residual (8.04e-6 relative to cycle-start at 0.5 CAD); isolated valve energy transitions close at diagnostic resolution, but later stages cannot repair a non-periodic state |
...
## 720-CAD open-system cycle gates

| Gate | Requirement | Notes |
|---|---:|---|
| Cycle mass accounting | Report initial/final mass, integrated intake/exhaust flow, leakage, and the residual; for a promoted fixed-step accounting run, relative residual must be below `1e-6` of cycle-start mass | A decreasing residual with timestep is diagnostic evidence, not closure by itself |
| Species accounting | Report valve and leakage species transfers and homologous composition change; require the existing periodic species tolerance before promotion | Preserve Cantera species order; no silent species aggregation |
| Energy accounting | Report enthalpy in/out, piston work, wall heat, and transition residual; require a closed energy/state map before enabling friction or crank dynamics | Gas-to-wall heat is subtracted from gas internal energy; valve and two-zone work quadratures must be identified |
| Periodic state | Mass, species, enthalpy, temperature, and speed at homologous crank angle meet the configured tolerances over repeated 720-CAD cycles | A transient/open-system result is not a stable-idle claim |
| Valve model scope | State reverse flow, clipping, timing, area, and manifold assumptions explicitly | The current accounting screen is one-way and lumped; it is not a calibrated valve train |
...
| Architecture-specific thermal evidence | Keep ringed heat-path/leakage coefficients separate from ringless ABC evidence; identify head/block temperatures as context only | Ringed coefficients or unpaired head/block temperatures cannot be injected into the ringless RC network |
| ABC profile evidence | Preserve manufacturer statements about intentional taper as qualitative unless a numeric profile and test conditions are recovered | A claimed taper magnitude without figure/page or drawing provenance cannot set a clearance |
| RPM event-time arithmetic | Define revolution and four-stroke periods from `60/N` and `120/N` seconds before forming `Lambda`, leakage fractions or chemistry timescale ratios | A 2x cycle-time error invalidates the derived envelope |
| Dimensionless nomenclature | Check units for Hersey/Sommerfeld/Stanton labels and retain plain timescale-ratio names when appropriate | A dimensionally invalid group cannot support a regime boundary |
| RPM regime claims | Require self-consistent state-dependent `P,T,mu,Nu` and ignition-delay calculations; treat Re/Fo crossings as hypotheses | No universal laminar/turbulent or CI/spark rpm is promoted from scaling alone |
| Coupled stability | Demonstrate nonlinear fixed points and Jacobian/eigenvalue behavior, including gap-conductance heat rejection, before using “saddle,” “bistable” or “runaway” language | Positive feedback sketches are not a stability result |
