#!/usr/bin/env python3
"""Build and run CFD-02 S2 medium-squish coarse case.

S2 keeps the CFD-01/S1 bore, stroke, CR, rpm, initial state and numerical
controls, while increasing squish area and reducing the TDC squish gap.  The
...
BOWL_RADIUS_M = 0.00300
SQUISH_WIDTH_M = 0.00125
SQUISH_GAP_M = 0.00035
# Exact constant-CR recess from the area-weighted clearance equation in Issue #7.
BOWL_RECESS_M = 0.0013788451646090532
WEDGE_DEG = 5.0
AZIMUTHAL_CELLS = 3
# Keep coarse cell widths close to the ~0.2 mm S1/CFD-01 screen.
BOWL_RADIAL_CELLS = 15
LAND_RADIAL_CELLS = 7
BOWL_LOWER_AXIAL_CELLS = 7
...
def block_mesh_dict() -> str:
    """Generate three conformal radial blocks with the S2 stepped bowl crown."""
    ...
    return f'''/* Generated by run_s2_cfd.py; S2 coarse constant-CR crown. */
FoamFile
{{
    format ascii;
    class dictionary;
...
def validate_case(case: Path, output: Path) -> tuple[list[dict[str, float]], dict[str, object], list[str]]:
    ...
    max_inventory = max(abs(item["tracer_inventory_error_percent"]) for item in history)
    ...
    if max_inventory > 100.0 * MAX_TRACER_INVENTORY_DRIFT_REL:
        failures.append(f"tracer inventory drift > {100.0 * MAX_TRACER_INVENTORY_DRIFT_REL:g}%")
    ...
    metrics = {
        ...
        "max_tracer_inventory_error_percent": max_inventory,
...
def write_target_summary(history: list[dict[str, float]], path: Path) -> None:
    ...
    fields = [
        "requested_crank_angle_deg_atdc",
        "sampled_crank_angle_deg_atdc",
        "tracer_mass_rms",
    ...
    with path.open("w", newline="") as handle:
        ...
        for target in targets:
            ...
            writer.writerow({
                ...
                "tracer_mass_rms": row["tracer_mass_rms"],
                "tracer_mass_rms_normalized": row["tracer_mass_rms_normalized"],
                "k_global_mix_1_s": row["k_global_mix_1_s"],
                "tau_global_mix_ms": row["tau_global_mix_ms"],
                "wall_shell_volume_fraction": row["wall_shell_volume_fraction"],
                "tracer_inventory_error_percent": row["tracer_inventory_error_percent"],
                "delta_c": row["delta_c"],
...
def main() -> None:
    ...
    metadata_path = case / "cfd02_s2_metadata.json"
