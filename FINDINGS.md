...
1. Regenerate flat fine, S1 coarse and S2 coarse with the converged tracer solve now in the base `fvSolution` (F21/F22), then redo the three-geometry normalized-RMS and 20%-mass-zone comparison under one set of numerics; only that comparison can decide squish.
2. Do not refine S1/S2, run S3, or couple squish schedules into Cantera until the regenerated comparison exists. The Issue #10 scalar defect itself is resolved (F21); a structurally bounded species-transport variant (F23) is optional and must be validated against the converged function-object result before replacing it.
